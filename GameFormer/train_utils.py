...
def planning_loss(plan, ego_future, acc_weight=0.0, yaw_weight=0.0, acc_limit=None, jerk_weight=0.0, dt=0.1):
    loss = F.smooth_l1_loss(plan, ego_future)
    loss += F.smooth_l1_loss(plan[:, -1], ego_future[:, -1])

    if acc_weight > 0:
        vel = torch.diff(plan[:, :, :2], dim=1) / dt  # [B, T-1, 2]
        acc = torch.diff(vel, dim=1) / dt  # [B, T-2, 2]
        acc_penalty = acc.pow(2).mean()
        if acc_limit is not None:
            exceed = torch.relu(acc.abs() - acc_limit)
            acc_penalty = acc_penalty + exceed.pow(2).mean()
        if jerk_weight and jerk_weight > 0:
            jerk = torch.diff(acc, dim=1) / dt
            acc_penalty = acc_penalty + jerk_weight * jerk.pow(2).mean()
        loss = loss + acc_weight * acc_penalty

    if yaw_weight > 0:
        yaw = plan[:, :, 2]
        yaw_rate = torch.diff(yaw, dim=1) / dt
        yaw_penalty = yaw_rate.pow(2).mean()
        loss = loss + yaw_weight * yaw_penalty
