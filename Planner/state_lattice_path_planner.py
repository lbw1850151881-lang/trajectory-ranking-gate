...
class LatticePlanner: 
    ...
    def post_process(self, path, ego_state):
        def _build_linear_path(points):
            points = np.asarray(points, dtype=float)
            if len(points) == 0:
                return np.empty((0, 4), dtype=float)

            if len(points) == 1:
                headings = np.array([ego_state.rear_axle.heading])
            else:
                dx = np.gradient(points[:, 0])
                dy = np.gradient(points[:, 1])
                headings = np.arctan2(dy, dx)
                if len(points) > 1 and not np.isfinite(headings[0]):
                    headings[0] = headings[1]
                headings = np.where(np.isfinite(headings), headings, ego_state.rear_axle.heading)

            curvature = np.zeros_like(headings)
            return np.stack([points[:, 0], points[:, 1], headings, curvature], axis=1)
        ...
        if index.size < 2:
            index = np.linspace(0, len(path) - 1, num=min(len(path), 2), dtype=int)

        x = path[:, 0][index]
        y = path[:, 1][index]

        valid_points = len(np.unique(x + 1j * y)) >= 2
        ...
        if valid_points:
            try:
                ...
                spline_path = _build_linear_path(path[index])
        else:
            spline_path = _build_linear_path(path[index])
